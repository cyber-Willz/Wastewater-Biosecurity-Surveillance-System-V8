...
*Formal account of how the wastewater biosurveillance system in `ww_biosec_v0_3_final_tar.gz` works: the physics it encodes, the statistics of its temporal detector, the spectral hypergraph theory behind its network score, and the Physics-Informed Neural Network (SHPINN) layer its schema is designed to feed.*
...
Math is written in LaTeX (`$…$`, `$$…$$`); view in a Markdown renderer with KaTeX/MathJax (GitHub, VS Code, Obsidian, Typora).
...
## 7. SHPINN: Spectral-Hypergraph Physics-Informed Neural Network [SPEC]

> **Status.** Nothing in this section exists in the archive. It is the formulation the schema is built to feed: `decay_rate_k` is exported per signal "for SHPINN PDE regression", `analyte.rs` names the PDE, and the illicit-substance category is described as "inverse source localisation". The proofs below are about this specification, not about running code.
...
## §6  Explosive Precursor & Manufacturing Residue Surveillance (v0.3)

### 6.1  Motivation

Wastewater-based epidemiology (WBE) for explosive substances exploits the
same principle as illicit-drug monitoring: manufacturing, handling, or
post-blast cleaning introduce trace residues into the sewer network at
concentrations detectable by LC-MS/MS, IC-MS/MS, or GC-MS/MS.  The technique
provides population-level early warning independent of physical-security
perimeters, complementing site-specific detection with a network-wide
chemical-signature fingerprint.

### 6.2  Target Compounds and Analytical Methods

| Compound / Class          | Wastewater Marker         | Method       | k (d⁻¹) | α       |
|---------------------------|---------------------------|--------------|---------|---------|
| RDX (cyclonite)           | Parent + ring-open pdts   | LC-MS/MS     | 0.02    | 0.10*   |
| HMX (octogen)             | Parent                    | LC-MS/MS     | 0.01    | 0.10*   |
| TNT                       | 2-ADNT + 4-ADNT           | LC-MS/MS     | 0.08    | 0.08→0.10* |
| PETN                      | Pentaerythritol (PENTA)   | LC-MS/MS     | 0.12    | 0.11    |
| TATP                      | DHPP (ring-open product)  | LC-MS/MS     | 0.28    | 0.24    |
| Ammonium nitrate (ANFO)   | Excess NO₃⁻ above baseline| IC           | 0.05    | 0.10*   |
| Perchlorate               | ClO₄⁻                     | IC-MS/MS     | 0.004   | 0.10*   |

*α clamped at lower bound of 0.10.

**Why transformation products?**  Parent energetics often sorb strongly to
suspended solids, photolyse near the surface, or are insufficiently polar for
reliable LC-MS/MS in complex wastewater matrices.  Stable transformation
products such as aminodinitrotoluenes (ADNT) and dihydroperoxypropane (DHPP)
remain dissolved and are better surrogates for source attribution.

### 6.3  Kinetics in the Transport PDE

The advection–diffusion–decay (ADR) equation used for all analytes:

    ∂C/∂t + v ∂C/∂x = D ∂²C/∂x² − k·C + f(x,t)

For explosive residues, k is the apparent first-order disappearance rate in
wastewater, integrating:

1. **Abiotic hydrolysis** — dominant for nitrate esters (PETN) and cyclic
   peroxides (TATP); faster at higher pH and temperature.
2. **Biodegradation** — RDX and HMX are slowly mineralised under aerobic
   conditions (co-metabolic); rate is low relative to HRT in most sewers.
3. **Sorption** — nitroaromatics (TNT) sorb to biofilm and settleable solids;
   the dissolved-phase k appears elevated relative to total-mass loss.
4. **Photolysis** — negligible in covered sewers; relevant only for surface
   waters receiving treated effluent.

The SHPINN inverse solver estimates the source field f(x) given concentration
observations y with the closed-form regularised least-squares:

    f̂ = argmin ‖Af − y‖²_{Σ⁻¹} + β fᵀ D^{1/2} Δ D^{1/2} f + γ‖f‖²

The hypergraph regulariser β fᵀΔf encodes the prior that the source is
spatially smooth over the catchment hypergraph, which is appropriate for
manufacturing facilities but not for point-source blast events.  For the
latter, the γ‖f‖² L₂ term alone (β = 0) is more appropriate.

### 6.4  Detection Thresholds and Alert Policy

All explosive-precursor analytes use `z_threshold = 3.5` (the industrial-
toxicant tier), because:

* Ambient baselines are near zero — a 3.5σ event represents a large absolute
  elevation from the detection limit, not merely a statistical artefact.
* False positives in this category carry high societal cost; the more
  conservative threshold reduces spurious escalation to law enforcement.
* Unlike infectious pathogens, a 48-hour delay in detection is acceptable
  given the persistence of the markers (long enough to still be detectable
  after confirmation sampling).

The `AMBER` severity tier is unreachable at `z_threshold = 3.5`
(per Thm 5.1 in §5), so all alerts enter directly at RED or CRITICAL.

### 6.5  Co-Elevation Signatures

Certain co-elevation patterns provide stronger attribution than any single
analyte:

| Signature                              | Likely Scenario                        | Action            |
|----------------------------------------|----------------------------------------|-------------------|
| RDX + HMX co-elevation                 | Military HE or plastic explosive       | Security escalation |
| TNT-products + HMX                     | Composition B or Semtex-type material  | Security escalation |
| TATP-DHPP isolated spike               | Improvised peroxide preparation        | Urgent response   |
| Perchlorate + AN excess                | ANFO or pyrotechnic oxidiser           | Regulatory inquiry |
| PETN + AN excess                       | Detonating cord + blasting material    | Mining/commercial check |

The analyst review layer (`ww_audit`) implements co-elevation logic by
examining alert bundles within the same time window and catchment zone.

### 6.6  Response Escalation Chain

    EXPLOSIVE_PRECURSOR alert raised
        ↓
    AI triage agent → preliminary classification (RDX/HMX, TATP, AN/Perchlorate)
        ↓
    Human analyst review (ww_audit::ReviewSession)
        ↓ (if confirmed)
    Escalate → National Security Council / Police Counter-Terrorism Unit
        ↓
    Confirmatory grab sampling + GC-MS/MS or LC-MS/MS re-analysis
        ↓
    Physical site inspection (warrant-based or cooperative)

### 6.7  Limitations

* **No synthesis-route inference** — the system detects presence of markers;
  it cannot infer formulation ratios, reaction stages, or precursor sources
  from wastewater data alone.
* **Matrix interference** — complex wastewater matrices may suppress or
  enhance LC-MS/MS signals; SPE enrichment and isotope-labelled internal
  standards are mandatory for reliable quantitation.
* **Spatial resolution** — the hypergraph Laplacian score identifies catchment-
  level anomalies; sub-catchment localisation requires the SHPINN inverse
  solver and dense monitoring-site coverage.
* **Seasonal flow variation** — dilution factors in tropical climates (Belize)
  can shift apparent concentrations by >10× during wet season; the EWMA
  baseline adapts but requires sufficient warm-up observations.
